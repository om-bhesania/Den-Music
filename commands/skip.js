    ...
    try {
      ...
      if (!shouldHandle) {
        ...
        if (bestBotId && bestBotId !== currentBotId) {
          console.log(`🚫 Bot ${currentBotId} (${interaction.client.user.username}) cannot handle skip command - Bot ${bestBotId} should handle voice channel ${voiceChannel.id}`);
      ...
      console.log(`🎵 Bot ${currentBotId} (${interaction.client.user.username}) handling skip command for voice channel ${voiceChannel.id}`);
